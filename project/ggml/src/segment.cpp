...
int image_segment_predict(SegmentModel *net, char *input_filename, char *output_filename)
{
    ...
    xxxx_test = net->get_output_tensor("seg_mask");
    if (tensor_valid(xxxx_test)) {
        tensor_show("********************** seg_mask", xxxx_test);
        tensor_destroy(xxxx_test);
    }


    xxxx_test = net->get_output_tensor("patch1");
    if (tensor_valid(xxxx_test)) {
        tensor_show("********************** patch1", xxxx_test);
        tensor_destroy(xxxx_test);
    }
    xxxx_test = net->get_output_tensor("patch2");
    if (tensor_valid(xxxx_test)) {
        tensor_show("********************** patch2", xxxx_test);
        tensor_destroy(xxxx_test);
    }
    xxxx_test = net->get_output_tensor("patch3");
    if (tensor_valid(xxxx_test)) {
        tensor_show("********************** patch3", xxxx_test);
        tensor_destroy(xxxx_test);
    }
    xxxx_test = net->get_output_tensor("patch4");
    if (tensor_valid(xxxx_test)) {
        tensor_show("********************** patch4", xxxx_test);
        tensor_destroy(xxxx_test);
    }

    xxxx_test = net->get_output_tensor("BlockMlp");
    if (tensor_valid(xxxx_test)) {
        tensor_show("********************** BlockMlp", xxxx_test);
        tensor_destroy(xxxx_test);
    }

    xxxx_test = net->get_output_tensor("seg_decoder");
    if (tensor_valid(xxxx_test)) {
        tensor_show("********************** seg_decoder", xxxx_test);
        tensor_destroy(xxxx_test);
    }
