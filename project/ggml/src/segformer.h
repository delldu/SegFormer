...
struct ConvModule {
    ...
    struct Conv2d conv;
    struct BatchNorm2d bn; // dell_add, relu ?
...
struct SegFormerHead {
    ...
    ggml_tensor_t* forward(struct ggml_context* ctx, std::vector<ggml_tensor_t *> xlist) {
        ...
        // Info: ********************** BlockMlp Tensor: 1x150x240x256
        // min: -7.3521, max: -2.9398, mean: -4.7168
        // tensor [mlp] size: [1, 150, 240, 256], min: -7.343958, max: -2.939925, mean: -4.71556
...
struct DWConv {
    ...
    ggml_tensor_t* forward(struct ggml_context* ctx, ggml_tensor_t* x, int H, int W) {
        ...
        // min: -2.7413, max: 1.6934, mean: -0.4951
        // tensor [mlp] size: [1, 960, 2048], min: -2.741753, max: 1.693425, mean: -0.495102
...
struct BlockMlp {
    ...
    ggml_tensor_t* forward(struct ggml_context* ctx, ggml_tensor_t* x, int H, int W) {
        ...
        // tensor [mlp] size: [1, 960, 512], min: -21.298813, max: 18.067135, mean: -0.061462

        // Info: ********************** BlockMlp Tensor: 1x1x960x512
        // min: -21.3229, max: 18.1213, mean: -0.0614
...
struct OverlapPatchEmbed {
    ...
    ggml_tensor_t* forward(struct ggml_context* ctx, ggml_tensor_t* x, int *H, int *W) {
        ...
        *H = (int)x->ne[1];
        ...
        // tensor [mlp] size: [1, 61440, 64], min: -0.880841, max: 0.975106, mean: -0.016517
...
struct SegmentModel : GGMLNetwork {
    ...
    ggml_tensor_t* forward(ggml_context_t* ctx, int argc, ggml_tensor_t* argv[]) {
        ...
        seg_logit = ggml_cont(ctx, seg_logit);
        ggml_set_name(seg_logit, "seg_decoder");
        ggml_set_output(seg_logit);
        // Info: ********************** seg_decoder Tensor: 1x150x240x256
        // min: -42.4641, max: -2.5731, mean: -23.4608
        // tensor [seg_decoder] size: [1, 150, 240, 256], min: -49.286793, max: 1.367848, mean: -24.109959
        ...
        GGML_ASSERT(B == 1);
        seg_logit = ggml_cont(ctx, ggml_permute(ctx, seg_logit, 1, 2, 0, 3)); // [W, H, C, B] -> [C, W, H, B]
        seg_logit = ggml_soft_max(ctx, seg_logit);
        seg_logit = ggml_cont(ctx, ggml_permute(ctx, seg_logit, 2, 0, 1, 3)); // [C, W, H, B] -> [W, H, C, B]

        seg_logit = ggml_cont(ctx, seg_logit);
        ggml_set_name(seg_logit, "seg_logit");
        ggml_set_output(seg_logit);
        // Info: ********************** seg_logit Tensor: 1x150x960x1024
        // min: 0.0000, max: 0.9063, mean: 0.0067
        // tensor [seg_logit] size: [1, 150, 960, 1024], min: 0.0, max: 1.0, mean: 0.006667


        // seg_logit = ggml_reshape_2d(ctx, seg_logit, C, W*H);
        ggml_tensor_dump("seg_logit1", seg_logit); // f32 [1280, 960, 150, 1]
        seg_logit = ggml_argmax_ext(ctx, seg_logit, 2/*dim on C*/);
        // ggml_tensor_label(ctx, seg_logit, "seg_mask");

        seg_logit = ggml_cont(ctx, seg_logit);
        ggml_set_name(seg_logit, "seg_mask");
        ggml_set_output(seg_logit);
        // Info: ********************** seg_mask Tensor: 1x1x960x1024
        // min: 0.0000, max: 10.0000, mean: 0.4940
        // tensor [seg_mask] size: [1, 1, 960, 1024], min: 0.0, max: 25.0, mean: 6.479004


        // seg_logit = ggml_cast(ctx, seg_logit, GGML_TYPE_F32);
        // seg_logit = ggml_clamp(ctx, seg_logit, 0, num_classes);
        // // seg_logit = ggml_reshape_2d(ctx, seg_logit, W, H);
